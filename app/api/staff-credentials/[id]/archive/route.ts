...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const credential = await StaffCredentialService.archive(id, validated.data, {
      user: auth.user,
      company: auth.company,
    });
    return successResponse(credential, 'Credential archived');
  } catch (error) {
    ...
    logger.error('POST /api/staff-credentials/[id]/archive failed', {
      id,
