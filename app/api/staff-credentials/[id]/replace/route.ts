...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await StaffCredentialService.replace(id, file, validated.data, {
      user: auth.user,
    ...
    return successResponse(result, 'New credential version uploaded');
  } catch (error) {
    ...
    logger.error('POST /api/staff-credentials/[id]/replace failed', {
      id,
