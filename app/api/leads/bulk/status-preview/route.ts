import { type NextRequest } from 'next/server';
import { resolveAuthContext } from '@/lib/api/middleware';
import { successResponse, errorResponse } from '@/lib/api/response';
import { bulkChangeLeadStatusSchema } from '@/lib/utils/validation';
import { LeadService } from '@/services/recruitment/LeadService';
import { PermissionDeniedError } from '@/lib/api/errors';
import { logger } from '@/lib/logger';

// Read-only dry run for the required pre-apply validation summary — never
// mutates. POST /api/leads/bulk/status re-validates independently at apply
// time regardless of what this returns.
export async function POST(request: NextRequest) {
  const auth = await resolveAuthContext(request);
  if (!auth.ok) return errorResponse('UNAUTHORIZED', 401);

  const body = await request.json().catch(() => null);
  if (!body) return errorResponse('VALIDATION_ERROR', 400, { message: 'Invalid JSON body' });

  const validated = bulkChangeLeadStatusSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const preview = await LeadService.previewBulkChangeStatus(
      { lead_ids: validated.data.lead_ids, new_status: validated.data.new_status },
      { user: auth.user, company: auth.company },
    );
    return successResponse(preview);
  } catch (error) {
    if (error instanceof PermissionDeniedError) return errorResponse('FORBIDDEN', 403);
    logger.error('POST /api/leads/bulk/status-preview failed', {
      error: error instanceof Error ? error.message : String(error),
    });
    return errorResponse('INTERNAL_ERROR', 500);
  }
}
