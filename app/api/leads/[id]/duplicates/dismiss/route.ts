...
import { dismissDuplicateSchema } from '@/lib/utils/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const validated = dismissDuplicateSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const dismissal = await LeadDuplicateService.dismissDuplicate(id, validated.data, {
      user: auth.user,
      company: auth.company,
    });
    return successResponse(dismissal, 'Duplicate dismissed', 201);
  } catch (error) {
    ...
    logger.error('POST /api/leads/[id]/duplicates/dismiss failed', {
      id,
