...
import { createLeadTaskSchema } from '@/lib/utils/validation';
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const tasks = await LeadService.listTasks(id, { user: auth.user, company: auth.company });
    return successResponse(tasks);
  } catch (error) {
    if (error instanceof PermissionDeniedError) return errorResponse('FORBIDDEN', 403);
    if (error instanceof NotFoundError) return errorResponse('NOT_FOUND', 404);
    logger.error('GET /api/leads/[id]/tasks failed', {
      id,
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const validated = createLeadTaskSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const task = await LeadService.createTask(id, validated.data, {
      user: auth.user,
    ...
    logger.error('POST /api/leads/[id]/tasks failed', {
      id,
      error: error instanceof Error ? error.message : String(error),
    });
    return errorResponse('INTERNAL_ERROR', 500);
