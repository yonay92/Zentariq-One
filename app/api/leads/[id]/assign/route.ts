...
import { assignLeadSchema } from '@/lib/utils/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const validated = assignLeadSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const lead = await LeadService.assign(id, validated.data, {
      user: auth.user,
      company: auth.company,
    });
    return successResponse(lead, 'Lead assigned');
  } catch (error) {
    ...
    logger.error('POST /api/leads/[id]/assign failed', {
      id,
