...
import { checkDuplicatesSchema } from '@/lib/utils/validation';
...
export async function POST(request: NextRequest) {
  ...
  const validated = checkDuplicatesSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const result = await LeadDuplicateService.checkDuplicates(validated.data, {
      user: auth.user,
    ...
    logger.error('POST /api/leads/check-duplicates failed', {
