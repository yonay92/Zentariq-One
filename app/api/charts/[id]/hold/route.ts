...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const chart = await ChartService.holdChart(id, { user: auth.user, company: auth.company });
    return successResponse(chart, 'Chart put on hold');
  } catch (error) {
    ...
    logger.error('POST /api/charts/[id]/hold failed', {
      id,
