...
import { reopenChartSchema } from '@/lib/utils/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const validated = reopenChartSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const chart = await ChartService.reopenChart(id, validated.data, {
      user: auth.user,
      company: auth.company,
    });
    return successResponse(chart, 'Chart reopened');
  } catch (error) {
    ...
    logger.error('POST /api/charts/[id]/reopen failed', {
      id,
