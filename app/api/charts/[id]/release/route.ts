...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const chart = await ChartService.releaseChart(id, { user: auth.user, company: auth.company });
    return successResponse(chart, 'Chart released');
  } catch (error) {
    ...
    logger.error('POST /api/charts/[id]/release failed', {
      id,
