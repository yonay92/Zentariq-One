...
import { markChartEnteredSchema } from '@/lib/utils/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const validated = markChartEnteredSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const chart = await ChartService.markEnteredInEdc(id, validated.data, {
      user: auth.user,
      company: auth.company,
    });
    return successResponse(chart, 'Chart marked Entered in EDC');
  } catch (error) {
    ...
    logger.error('POST /api/charts/[id]/mark-entered failed', {
      id,
