...
import { overrideDocumentStatusSchema } from '@/lib/utils/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const validated = overrideDocumentStatusSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const document = await RegulatoryDocumentService.overrideStatus(id, validated.data, {
      user: auth.user,
      company: auth.company,
    });
    return successResponse(document, 'Document status overridden');
  } catch (error) {
    ...
    logger.error('POST /api/regulatory/documents/[id]/override-status failed', {
      id,
