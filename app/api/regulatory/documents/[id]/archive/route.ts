...
import { archiveDocumentSchema } from '@/lib/utils/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const validated = archiveDocumentSchema.safeParse(body);
  if (!validated.success) {
    return errorResponse('VALIDATION_ERROR', 400, { issues: validated.error.issues });
  }

  try {
    const document = await RegulatoryDocumentService.archive(id, validated.data, {
      user: auth.user,
      company: auth.company,
    });
    return successResponse(document, 'Document archived');
  } catch (error) {
    ...
    logger.error('POST /api/regulatory/documents/[id]/archive failed', {
      id,
